...
export default function TeamDiscoveryResult({task,run,onAdopt,onRefine}:{task:TeamTask;run:TeamRun;onAdopt:()=>Promise<void>;onRefine:(text:string)=>Promise<void>}){
 ...
 return <section className="team-discovery-result" aria-label={tr('方向与下一步建议')}>
  ...
  <Markdown text={result.proposal?.body||result.text}/>
  ...
  {!result.proposal&&<p className="team-note">{tr('模型未提供可直接带入的任务建议，你仍可阅读上方结果并手动填写下一步。')}</p>}
  <p className="team-note">{tr('选择下一步会结束本轮梳理，不表示同意执行方案。')}</p>
  {error&&<p role="alert">{error}</p>}
  <button className="btn primary" disabled={busy} onClick={async()=>{if(busy)return;setBusy(true);setError('');try{await onAdopt();}catch(e){setError(String(e instanceof Error?e.message:e));}finally{setBusy(false);}}}>{tr('修改建议，建立下一项任务')}</button>
