...
export default function MilestonePanel({ items, steps = [], requirements = [] }: { items?: Milestone[]; steps?: ToolStep[]; requirements?: DeliveryRequirement[] }) {
  ...
  return <details className="milestone-panel" open={complete < items.length}>
    ...
    <ol>{items.map(m => <li key={m.id} className={`milestone-${m.status}`}>
      <span className="milestone-check" aria-label={{ completed:'已完成',in_progress:'进行中',pending:'待完成',verifying:'待质检',blocked:'受阻' }[m.status]}>
        {{ completed:'✓',in_progress:'◉',pending:'○',verifying:'◷',blocked:'!' }[m.status]}
      </span>
      <div><span className="milestone-title">{m.title}</span>
        {m.status === 'verifying' ? <small>待质检 · 尚未完成</small> : null}
        {m.acceptance ? <small>验收：{m.acceptance}</small> : null}
        {m.note ? <small>{m.note}</small> : null}
        {requirements.some(r=>r.milestoneId===m.id) ? <details className="milestone-evidence"><summary>质检记录</summary>
          {requirements.filter(r=>r.milestoneId===m.id).map(r=><div key={r.id}><strong>{r.title}</strong>
            {[...(r.verificationHistory??[]),...(r.verification?[r.verification]:[])].slice(-6).map((v,i)=><p key={i}>{v.method==='program'?'程序检查':'模型复核'} · {{passed:'通过',failed:'未通过',unverifiable:'无法核验'}[v.status]}：{v.detail}</p>)}
            {!r.verification ? <p>等待检查</p> : null}</div>)}
        </details> : null}
        {m.history?.length ? <details className="milestone-evidence"><summary>变更记录</summary>{m.history.map((h,i)=><p key={i}>{new Date(h.at).toLocaleString()} · {h.title}：{h.reason}</p>)}</details> : null}
