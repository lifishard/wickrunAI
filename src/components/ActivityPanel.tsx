...
export default function ActivityPanel({ messages, onHide }: { messages: ChatMessage[]; onHide: () => void }) {
  ...
  return <aside className={`activity-panel${collapsed ? ' collapsed' : ''}`} aria-label={t('任务动态')}>
    <header className="activity-head">
      <strong>{t('任务动态')}</strong>
      <button className="btn sm ghost" aria-expanded={!collapsed} onClick={() => setCollapsed(v => !v)}>{t(collapsed ? '展开' : '收起')}</button>
      <button className="btn sm ghost" onClick={onHide} aria-label={t('隐藏任务动态')}>{t('隐藏')}</button>
    </header>
    ...
    {!collapsed ? <div className="activity-body">
      ...
      {available.length > 1 ? <label className="activity-selector">{t('查看记录')}
        <select aria-label={t('查看哪轮任务动态')} value={available.some(m => m.id === selected) ? selected : 'latest'} onChange={e => setSelected(e.target.value)}>
          <option value="latest">{t('跟随最新')}</option>
          {[...available].reverse().map((m, i) => <option key={m.id} value={m.id}>{t('第 {n} 轮', { n: available.length - i })} · {m.model ?? t('助手')}</option>)}
        </select>
      </label> : null}
      <StepTrace key={answer.id} steps={steps} live={Boolean(answer.pending)} />
      {progress.saved ? <details className="activity-progress"><summary>{t('已保存的进度')}</summary><p>{progress.saved}</p></details> : null}
    </div> : null}
  </aside>;
}
