...
const EN: Record<string, string> = {
  ...
  '同一 Google 账号可在桌面版与网页版查看聊天、项目、技能和任务记录，并使用自己的 API 密钥。': 'Use the same Google account on desktop and web to access your chats, projects, skills, task history and API keys.',
  '云同步尚未配置。请在 Railway 添加数据库和加密变量。': 'Cloud sync is not configured yet. Add the database and encryption variable in Railway.',
  '请先暂停正在运行的任务，再同步或切换账号。': 'Pause running tasks before syncing or switching accounts.',
  '浏览器会打开 Google 登录。请核对两处校验码一致，再批准此桌面程序。': 'Google sign-in opens in your browser. Check that both verification codes match before approving this desktop app.',
  '登录完成，重启进入账号工作区': 'Sign-in complete. Restart to open your account workspace',
  '退出并返回未登录工作区': 'Sign out and return to the guest workspace',
  '导入未登录时的本地内容': 'Import local guest content',
  '将此设备未登录时的内容和 API 密钥复制到当前账号。原本地内容保留；相同编号的账号内容及已有密钥不会被覆盖。': 'Copy this device’s guest content and API keys into this account. Original local data is retained; existing cloud keys are not overwritten.',
  '导入到此账号': 'Import into this account',
  '发现 {n} 条冲突；其他内容会正常合并。本机会先保留恢复副本。': 'Found {n} conflicts. Other changes will be merged normally. A local recovery copy is saved first.',
  '冲突使用本机版本': 'Use local versions for conflicts',
  '冲突使用云端版本': 'Use cloud versions for conflicts',
  '空闲时自动同步。API 密钥通过独立加密通道保存；新设备的技能与定时任务需要手动启用，本地工具权限不随账号同步。': 'Sync runs automatically while idle. API keys use separate encrypted storage. Enable skills and scheduled tasks on each new device; local tool permissions do not sync.',
  '账号与本地同步记录不匹配，已停止同步。': 'The account does not match the local sync record. Sync has stopped.',
  '内容已变化，请重新同步并检查冲突。': 'Content has changed. Sync again and review the conflicts.',
  '任务正在运行，云端已保存；空闲后会继续合并本机内容。': 'A task is running. Cloud data is saved; local changes will be merged once idle.',
  '云端同步完成': 'Cloud sync complete',
  '同一内容在两端都有修改，请选择冲突版本。': 'The same content changed on both devices. Choose which conflicting versions to keep.',
  '查看任务记录（{n}）': 'View task history ({n})',
  '这里是跨设备保存的历史记录，不会在此恢复或执行本机进程。': 'These are saved task records from your devices. Local processes are not resumed or executed here.',
  '使用 Google 登录': 'Sign in with Google',
  '此账号的 API 密钥会加密保存到云端，并在已登录的设备上使用。': 'API keys for this account are encrypted in cloud storage and available on signed-in devices.',
  '本地工作区 · 支持云同步': 'Local workspace · Cloud sync supported',
  '账号内容可通过云端同步在设备之间访问。未登录时的本地记录需在「云端同步」中选择导入。': 'Access your account content across devices with cloud sync. Import guest records from the Cloud sync dialog.',
  ...
  'API 模型': 'API model',
  '本机 AI': 'On-device AI',
  '请先回答待处理的问题': "Please answer the pending question first",
  '本机连接需要使用桌面版。': "On-device connections need the desktop app.",
  '你已核实并跳过先前未确认的操作；本次没有重新执行。': "You verified and skipped the earlier unconfirmed operation; it was not run again.",
  '上次本机操作尚未确认。请核实产物后选择跳过或明确重试，避免重复执行。': "The last on-device operation is not confirmed yet. Check its output, then choose skip or retry explicitly so nothing runs twice.",
  '此前 API 工具的执行结果尚未确认，请先在原连接中核实该操作，再切换本机连接。': "An earlier API tool result is not confirmed yet. Verify it in the original connection before switching to an on-device connection.",
  '问题回答尚未准备好。': "The answer to the question is not ready yet.",
  '已暂停，尚未派发本机请求。': "Paused before the on-device request was sent.",
  'Grok 正在思考': "Grok is thinking",
  '等待 Grok 返回 · {n} 秒未收到新动态 · 可随时暂停': "Waiting for Grok · no update for {n} s · you can pause any time",
  'Grok 正在执行操作': "Grok is running an operation",
  '正在接收官方客户端回复': "Receiving the official client’s reply",
  'Grok 请求修改文件': "Grok asks to change a file",
  '官方客户端请求执行操作': "The official client asks to run an operation",
  '官方客户端正在等待操作确认': "The official client is waiting for you to confirm an operation",
  '正在等待官方客户端返回结果…': "Waiting for the official client’s result…",
  '已暂停并保存当前执行现场': "Paused; the current progress is saved",
  '图片附件数据缺失，请重新添加图片后继续；原对话已保留。': "Image attachment data is missing. Add the image again to continue; the conversation is kept.",
  '本机客户端代码改动': "Code changes by the on-device client",
  '已暂停并保存当前执行现场；尚未确认的本机操作需要核实': "Paused and saved. An unconfirmed on-device operation needs checking",
  '本机客户端的问题格式无效：{error}': "The on-device client sent a question in an invalid format: {error}",
  '已有问题尚未回答，已保存独立工作结果；等待回答后继续': "A question is still unanswered. Independent work is saved; it continues once you answer",
  '官方客户端已暂停（{status}），已有内容已保留。': "The official client paused ({status}); what it produced so far is kept.",
  '已完成可独立进行的工作，等待用户回答': "Independent work is done; waiting for your answer",
  '当前连接无法核验文件': "This connection cannot verify files",
  '本机客户端仅返回了计划，尚未确认完成。': "The on-device client only returned a plan; completion is not confirmed.",
  '本机客户端只返回了下一步计划，任务尚未完成。请继续本轮以核实进度；应用没有自动重发可能已执行的本机操作。': "The on-device client only returned its next steps; the task is not done. Continue this turn to check progress. The app did not resend on-device operations that may already have run.",
  '本阶段接力次数已到，进度和待回答的问题已保存': "This stage used up its hand-offs; progress and the pending question are saved",
  '执行记录写入失败，已停止；请核实本机客户端的运行状态。': "Saving the run record failed, so it stopped. Check what the on-device client is doing.",
  '当前任务已停止': "The current task has stopped",
  '新输入和原任务现场已保存。Claude Desktop 中的原操作可能仍在运行，请先在官方应用核实结果，再新窗口交接；未自动重复派发。': "Your new input and the task’s progress are saved. The original operation may still be running in Claude Desktop; check the result there, then hand off in a new window. Nothing was sent again automatically.",
  'Claude Desktop 连接需要桌面版。': "The Claude Desktop connection needs the desktop app.",
  'Claude Desktop 交接暂不自动传图片。图片已保留，可在官方应用添加，或切换 Claude Code、Codex 等支持图片的连接。': "Hand-offs to Claude Desktop do not carry images yet. The images are kept; add them in the official app, or switch to Claude Code, Codex or another connection that supports images.",
  'Claude Desktop 的交接材料超过 24000 字符。请使用新对话明确本次目标，或改用 Claude Code / API 模型处理长上下文；原文未裁剪。': "The hand-off for Claude Desktop is over 24,000 characters. Start a new chat with a clear goal, or use Claude Code or an API model for long context; nothing was cut.",
  '已停止 Claude Desktop 交接。': "Stopped the Claude Desktop hand-off.",
  '任务已排队，等待 Claude 领取（在 Claude 里说「领取灯芯AI 任务」，或让它的定时任务自动领取）。进度和结果会回到此处。': "Task queued, waiting for Claude to claim it (say “claim the wickrunAI task” in Claude, or let its scheduled task pick it up). Progress and results will come back here.",
  '已打开 Claude Desktop。请在官方应用发送预填的话并授权连接器，Claude 会领取此任务；进度和结果会回到此处。': "Opened Claude Desktop. Send the prefilled message there and approve the connector; Claude will claim this task, and progress and results will come back here.",
  '原 Claude Desktop 任务记录已不可用，未自动重发。': "The original Claude Desktop task record is no longer available; it was not resent.",
  'Claude Desktop 已回传，但内容仍只有计划，任务尚未确认完成。': "Claude Desktop replied, but only with a plan; the task is not confirmed as done.",
  'Claude Desktop 任务已取消，已有记录保留。': "The Claude Desktop task was cancelled; existing records are kept.",
  '等待 Claude Desktop 达到本阶段时间上限，可继续检查原任务；未自动重发。': "Waiting for Claude Desktop reached this stage’s time limit. You can keep checking the original task; nothing was resent.",
  '已停止等待 Claude Desktop，已有结果保留。': "Stopped waiting for Claude Desktop; results so far are kept.",
  '交接记录保存失败，请先核实 Claude Desktop 中的原任务。': "Saving the hand-off record failed. Check the original task in Claude Desktop first.",
  '已开启「逐项修改前确认」：{reason}，本轮没有进入工作模式。可以改用对话模式、API 模型或 Grok，或在设置 → 工具里关闭「逐项修改前确认」。': "“Confirm each change first” is on: {reason}, so this turn did not use work mode. Switch to chat mode, an API model or Grok, or turn off “Confirm each change first” in Settings → Tools.",
  '{who}报告任务受阻：{reason}': "{who} reported the task is blocked: {reason}",
  '未说明原因': "No reason given",
  '经中继交出的任务暂不传图片。图片已保留，可改用支持图片的 API 模型。': "Tasks handed out through the relay do not carry images yet. The images are kept; switch to an API model that supports images.",
  '交接材料超过 200000 字符，请新开对话明确本次目标。': "The hand-off is over 200,000 characters. Start a new chat with a clear goal.",
  '任务已排队，等待 Claude 领取（在 Claude 里说「领取灯芯AI 任务」，或让它的定时任务自动领取）。': "Task queued, waiting for Claude to claim it (say “claim the wickrunAI task” in Claude, or let its scheduled task pick it up).",
  '任务已排队，等待「{name}」领取。': "Task queued, waiting for “{name}” to claim it.",
  '任务已排队，等待已登录的桌面版或 CLI 连接器领取。': "Task queued, waiting for a signed-in desktop app or CLI connector to claim it.",
  '原任务记录已不可用，未自动重发。': "The original task record is no longer available; it was not resent.",
  'Claude 已领取，正在处理…': "Claude claimed it and is working…",
  '本机已领取，正在处理…': "Your computer claimed it and is working…",
  '任务已取消，已有记录保留。': "The task was cancelled; existing records are kept.",
  '等待达到本阶段时间上限；任务仍在队列里，可稍后接着查看，未自动重发。': "Waiting reached this stage’s time limit. The task is still queued; check back later. Nothing was resent.",
  '已停止等待，已有结果保留。': "Stopped waiting; results so far are kept.",
  '任务记录保存失败，请刷新后查看原任务。': "Saving the task record failed. Refresh and check the original task.",
  '{reason}，{n} 秒后继续': "{reason}, continuing in {n} s",
  '等待你确认此操作，已有进度保留': "Waiting for you to confirm this operation; progress is kept",
  '正在整理较早上下文，原始记录保留，可随时暂停': "Condensing earlier context; the original record is kept and you can pause any time",
  '整理上下文等待额度，{n} 秒后继续': "Condensing context is waiting for quota, continuing in {n} s",
  '较早上下文已整理，继续执行': "Earlier context condensed; continuing",
  '本次摘要未通过检查，保留原文并使用工具结果缩减': "This summary failed its check; keeping the original and trimming tool results instead",
  '没有可用的语义压缩结果，保留原文并继续执行': "No usable condensed summary; keeping the original and continuing",
  '等待调用额度，{n} 秒后继续；已完成步骤保留': "Waiting for quota, continuing in {n} s; finished steps are kept",
  '正在缩小本轮上下文，完整证据仍保留': "Shrinking this turn’s context; the full evidence is still kept",
  '此模型不支持提问卡片，正在继续普通聊天': "This model does not support question cards; continuing as a normal chat",
  '正在等待临时子代理完成，已有主任务进度保留…': "Waiting for a temporary sub-agent to finish; the main task’s progress is kept…",
  '任务尚未交付，正在继续已授权的工作…': "The task is not delivered yet; continuing the authorized work…",
  '继续等待调用额度恢复': "Still waiting for quota to recover",
  '调用额度暂时不足': "Quota is temporarily exhausted",
  '连接暂时中断，正在自动恢复': "The connection dropped; recovering automatically",
  '云账号登录已失效，请重新登录。': "Your cloud account sign-in has expired. Please sign in again.",
  '云端暂不可用。': "The cloud service is unavailable right now.",
  '请先登录云账号，再允许网页版使用本机 AI。': "Sign in to your cloud account before allowing the web app to use on-device AI.",
  '连接官方 ChatGPT 账号后即可选择订阅提供的模型。': "Connect your official ChatGPT account to choose the models your subscription provides.",
  '已连接官方 ChatGPT 账号，模型列表来自本机 Codex。': "Connected to your official ChatGPT account. The model list comes from Codex on this computer.",
  'Claude 账号登录': "Claude account sign-in",
  '自定义 API：{url}': "Custom API: {url}",
  '已配置的本机网关已响应。': "The configured local gateway responded. ",
  'Claude Code CLI 已就绪。连接来源：{source}。{gateway}尚未发送模型请求；可用模型以你的服务配置为准。': "Claude Code CLI is ready. Connection: {source}. {gateway}No model request has been sent yet; available models depend on your service setup.",
  'Claude Code CLI 可用。Claude Code 自己配置的本机网关 {url} 需要检查：{reason}。这只影响「沿用 Claude Code 自己的配置」这个大脑；选本机订阅或 wickrunAI 路由不受影响。': "Claude Code CLI is available. The local gateway {url} in Claude Code’s own configuration needs a check: {reason}. This only affects the “Claude Code’s own configuration” brain; your local subscription and wickrunAI routes still work.",
  'Claude Code 授权未通过检查，请检查现有账号或 API 配置后重新检测。': "Claude Code authorization did not pass the check. Review your account or API setup, then check again.",
  'Claude Code 配置的默认模型': "Claude Code configured default model",
  '{id} · 配置别名': "{id} · configured alias",
  '已连接官方 Grok 账号，模型列表来自本机 Grok Desktop。': "Connected to your official Grok account. The model list comes from Grok on this computer.",
  '已连接 Kimi ACP；执行范围受客户端能力与授权限制。': "Connected to Kimi ACP. What it can do is limited by the client’s capabilities and your permissions.",
  'Grok ACP 要求登录；请先完成官方 grok login，或点击登录 Grok，再重新检测。': "Grok ACP needs sign-in. Run the official grok login or press Sign in to Grok, then check again.",
  'Kimi ACP 要求登录；请先在官方客户端运行 kimi login，再重新检测。': "Kimi ACP needs sign-in. Run kimi login in the official client, then check again.",
  '客户端未完成连接，请检查官方客户端版本、登录或网络后重新检测。': "The client did not finish connecting. Check the official client version, sign-in or network, then check again.",
  '无法发起官方登录，请检查 Grok 客户端。': "Could not start the official sign-in. Check the Grok client.",
  '已启动官方 grok login，完成登录后点击重新检测。': "Started the official grok login. After signing in, press Check again.",
  '已打开官方登录页面，完成登录后点击重新检测。': "Opened the official sign-in page. After signing in, press Check again.",
  '无法发起官方登录，请检查 Codex 客户端。': "Could not start the official sign-in. Check the Codex client.",
  '重启后连接检查失败，请手动重新检测。': "The connection check after restart failed. Please check again manually.",
  'Claude 恢复检查失败，请核对程序路径和用户路由配置。': "The Claude recovery check failed. Review the program path and your route settings.",
  '未知连接器': "Unknown connector",
  '选中的是 Claude Desktop。请为 Claude Code 选择 CLI 原生程序。': "That is Claude Desktop. Choose the native Claude Code CLI program instead.",
  '已发现 Kimi 桌面应用，但未找到提供 ACP 接口的 Kimi Code CLI。请安装官方 Kimi Code CLI，或选择其 kimi.exe 后重新检测。': "Found the Kimi desktop app, but not the Kimi Code CLI that provides the ACP interface. Install the official Kimi Code CLI, or choose its kimi.exe, then check again.",
  '已发现 Grok Desktop，但未找到提供 ACP 接口的官方 grok CLI。请安装官方 Grok CLI，或选择其 grok 程序后重新检测。': "Found Grok Desktop, but not the official grok CLI that provides the ACP interface. Install the official Grok CLI, or choose its grok program, then check again.",
  '未找到 {client} 官方原生客户端，请安装后重新检测，或选择程序位置。': "The official {client} client was not found. Install it and check again, or choose where the program is.",
  'Claude Code 配置的本机服务需要检查。': "The local service in Claude Code’s configuration needs a check. ",
  '一键恢复仅支持明确配置为 OmniRoute 的本机 HTTP /v1 地址。': "One-click recovery only supports a local HTTP /v1 address explicitly set up as OmniRoute.",
  '未找到已安装的 OmniRoute。请先安装官方 OmniRoute CLI，再点恢复；应用不会自动下载程序。': "OmniRoute is not installed. Install the official OmniRoute CLI, then press recover; the app never downloads programs by itself.",
  '已找到 OmniRoute，但没有找到兼容的 Node.js（22.22.2+ 的 22 版，或 24–26 版）。请先修复 Node.js 安装。': "Found OmniRoute, but no compatible Node.js (22.22.2+ on 22, or 24–26). Fix the Node.js installation first.",
  '本机服务跳转到了其他地址，无法确认是 OmniRoute。请核对网关地址。': "The local service redirected elsewhere, so it could not be confirmed as OmniRoute. Check the gateway address.",
  '本机端口已有服务响应，但未能确认是 OmniRoute；没有重启或替换它。': "A service answered on the local port but could not be confirmed as OmniRoute; it was not restarted or replaced.",
  '这个端口运行的是其他服务，不能用 OmniRoute 覆盖。请核对 Base URL。': "Another service runs on this port and cannot be replaced with OmniRoute. Check the Base URL.",
  '网关已响应，但本地凭据无法解锁。请检查凭据，未重启服务。': "The gateway responded, but the local credential could not be unlocked. Check the credential; the service was not restarted.",
  'OmniRoute 已启动，但当前 API 凭据未通过验证。请检查密钥；重启服务无法修复凭据。': "OmniRoute is running, but the current API credential was rejected. Check the key; restarting will not fix it.",
  'OmniRoute 已响应，但模型接口返回 HTTP {status}。请查看网关状态；未重复启动。': "OmniRoute responded, but its models endpoint returned HTTP {status}. Check the gateway status; it was not started again.",
  'OmniRoute 已响应，但模型接口返回异常。请检查网关日志。': "OmniRoute responded, but its models endpoint returned something unexpected. Check the gateway log.",
  '本机网关连接正常。可以接着跑或重新发送。': "The local gateway is working. You can continue or send again.",
  '没有取得本机网关的有效响应。': "No valid response from the local gateway.",
  '已通过 127.0.0.1 恢复连接，并避开 localhost 的地址解析差异。': "Reconnected through 127.0.0.1, avoiding localhost resolution differences.",
  '端口仍被占用或连接状态不明，服务可能正在启动或暂时无响应。请稍后再试；没有强制结束现有进程。': "The port is still in use or its state is unclear; the service may be starting or briefly unresponsive. Try again shortly; no running process was killed.",
  '后台启动失败，请检查 OmniRoute 和 Node.js 安装。': "Background start failed. Check the OmniRoute and Node.js installations.",
  'OmniRoute 启动进程退出（{code}），请检查网关日志。': "The OmniRoute start process exited ({code}). Check the gateway log.",
  '无法启动后台网关，请检查 OmniRoute 安装。': "Could not start the background gateway. Check the OmniRoute installation.",
  '已在后台启动 OmniRoute，连接已恢复。可以接着跑或重新发送。': "Started OmniRoute in the background and the connection is back. You can continue or send again.",
  '已发起后台启动，但网关尚未就绪。稍后再次检测；不会重复启动仍在运行的进程。': "A background start was requested but the gateway is not ready yet. Check again later; a running process will not be started twice.",
  '这份 API 凭据已不存在，请重新选择。': "This API credential no longer exists. Choose again.",
  'Claude Code 自己配置的本机网关没有响应，这个大脑现在用不了：可以改选本机订阅或 wickrunAI 路由，或点下面的「修复连接」。': "The local gateway in Claude Code’s own configuration is not responding, so this brain cannot be used right now. Pick your local subscription or a wickrunAI route instead, or press Repair connection below.",
  '{client} 自定义大脑模型': "{client} custom brain model",
  '网关需检查': "Gateway needs a check",
  '自定义模型 ID…': "Custom model ID…",
  '输入模型 ID': "Enter a model ID",
  '这条路由还没有模型列表：去设置 → API 凭据刷新模型，或直接输入模型 ID。': "This route has no model list yet. Refresh its models in Settings → API credentials, or enter a model ID directly.",
  '选择模型': "Choose a model",
  '交给 Claude Desktop 的任务，由 Claude Desktop 用它当前选中的模型处理。': "Tasks handed to Claude Desktop run on whichever model is currently selected in Claude Desktop.",
  '以后不再提示': "Don’t show this again",
  '在 Claude Desktop 里选模型': "Choose the model in Claude Desktop",
  '想在灯芯AI 里直接控制 Claude 的模型和思考强度，请改用「Claude Code」连接：它可以选大脑路由、本机订阅和思考强度。': "To control Claude’s model and effort from wickrunAI, use the Claude Code connection instead: it lets you pick the brain route, your local subscription and the effort.",
  '模型和思考强度只能在 Claude Desktop 的输入框里选择和调整，灯芯AI 的模型、思考强度和路由设置对它不生效。': "The model and effort can only be chosen and changed in Claude Desktop’s message box. wickrunAI’s model, effort and route settings do not apply to it.",
  '模型和思考强度只能在 Claude Desktop 里选择和调整，灯芯AI 这里的模型设置对它不生效。': "The model and effort can only be chosen and changed in Claude Desktop; the model settings here do not apply to it.",
  '知道了，开始使用': "Got it, use it",
  '账号、额度和工具授权也都在 Claude Desktop 里管理。': "Your account, usage limits and tool permissions are also managed in Claude Desktop.",
  '本机客户端里只有 Grok 能在开启后进入工作模式：它的每处修改先展示差异，批准后才写入，命令不运行。Claude Code、Codex、Kimi 和 Claude Desktop 会先弹窗说明，这一轮不进入工作模式。': "Among on-device clients, only Grok can use work mode while this is on: each change shows a diff first and is written only after approval, and commands do not run. Claude Code, Codex, Kimi and Claude Desktop show an explanation first and skip work mode for that turn.",
  'Claude Code 会直接写文件和运行命令，改动不会先交给 wickrunAI 审核': "Claude Code writes files and runs commands directly, so its changes never reach wickrunAI for review first",
  'Claude Desktop 在它自己的应用里改文件，改动不会先交给 wickrunAI 审核': "Claude Desktop changes files inside its own app, so its changes never reach wickrunAI for review first",
  'Codex 在沙箱里可以直接改工作目录里的文件、运行命令，改动不会先交给 wickrunAI 审核': "Codex can change files in the working directory and run commands inside its sandbox, so its changes never reach wickrunAI for review first",
  'Kimi 申请修改时只给出文件路径、不带修改内容，无法在批准前展示差异': "Kimi asks to edit with only a file path and no content, so there is no diff to show before approval",
  '这个客户端的改动无法在写入前审核': "This client’s changes cannot be reviewed before they are written",
  '以下改动尚未写入文件。批准后生效；拒绝或关闭窗口保留原文件。': "These changes have not been written yet. They take effect when you approve; rejecting or closing keeps the original file.",
  '可以这样继续': "Ways to continue",
  '在设置 → 工具里关闭「逐项修改前确认」：改动仍会记录在代码改动面板，可逐项回退。': "Turn off “Confirm each change first” in Settings → Tools. Changes are still recorded in the code changes panel and can be reverted one by one.",
  '已开启「逐项修改前确认」：每处文件改动都要先给你看差异、批准后才写入。': "“Confirm each change first” is on: every file change must show you a diff and be approved before it is written.",
  '批准后由 Grok 写入；本轮结束时 wickrunAI 会核对实际写入是否与这里一致，不一致会在代码改动里提示。': "Grok writes the file after you approve. At the end of the turn wickrunAI checks the result matches this diff and flags any difference in code changes.",
  '换成 API 模型：用 wickrunAI 自己的文件工具，每处改动逐项审核。': "Switch to an API model: wickrunAI’s own file tools let you review every change.",
  '换成 Grok：每处修改先弹出差异，批准后才写入；审核模式下不运行命令。': "Switch to Grok: each change shows a diff first and is written only after approval. Commands do not run in review mode.",
  '改用对话模式：只聊天，不改文件。': "Switch to chat mode: talk only, no file changes.",
  '知道了': "Got it",
  '这一轮不能进入工作模式': "Work mode is unavailable for this turn",
  '逐项修改前确认已开启': "Confirm each change first is on",
  '，这一轮不能进入工作模式，已暂停，已有记录保留。': ", so this turn cannot use work mode. It is paused and everything so far is kept.",
  '未开启': "Off",
  '正在处理网页任务': "Working on a web task",
  '网页版可用': "Available to the web app",
  '正在连接…': "Connecting…",
  '允许网页版使用': "Allow the web app",
  '打开后，用同一个云账号登录的网页版可以把对话交给这台电脑上已连接的客户端。只做对话，不开放文件和命令。': "When on, the web app signed in to the same cloud account can hand chats to the clients connected on this computer. Chat only; no file or command access.",
  '先在左下角登录云账号。': "Sign in to your cloud account at the bottom left first.",
  '允许网页版使用本机 AI': "Allow the web app to use on-device AI",
  '网页版可见：{list}': "Visible to the web app: {list}",
  '还没有就绪的客户端。先在上面连接 Claude Code、Codex、Kimi 或 Grok。': "No client is ready yet. Connect Claude Code, Codex, Kimi or Grok above first.",
  '正在处理：{title}': "Working on: {title}",
  ...
  '完成': 'Done',
  // 路由组
  '已选用路由组「{name}」：当前路由 {model}，失灵时按组内顺序交接。': 'Route group “{name}” selected: current route is {model}; failover follows the group order.',
  '未命名路由组': 'Untitled route group',
  ...
  '选用路由组': 'Use route group',
  '切到这一组的第一条路由，失灵时按组内顺序交接': 'Switch to the first route in this group; failover follows the group order',
  '还没有路由组，去设置里建一组': 'No route groups yet. Create one in Settings',
  '{name} 副本': '{name} copy',
  '从应用全局接力名单新建': 'Create from the app-wide failover list',
  '例如：免费 / 写代码': 'For example: Free / Coding',
  '全局接力名单': 'App-wide failover list',
  '再点一次删除': 'Click again to delete',
  '凭据已删除': 'Credential deleted',
  '分组和排序完全由你决定，程序不判断哪条路由付费，也不自动调整顺序。': 'You decide the groups and their order. The app does not judge which routes are paid and never reorders them.',
  '删除后，已选用这一组的对话和项目会保留选用时的顺序，不会丢失接力名单。': 'After deletion, conversations and projects that use this group keep the order saved when they selected it, so no failover list is lost.',
  '可选，只给自己看': 'Optional, for your reference only',
  '备注': 'Note',
  '应用全局正在使用': 'In use app-wide',
  '当前路由': 'Current route',
  '把常用的路由组合存成一组，比如按任务分「写代码」「翻译」，或者把免费路由放进「免费」。': 'Save route combinations you use often as a group, for example by task (“Coding”, “Translation”) or put free routes in “Free”.',
  '新建路由组': 'New route group',
  '组内顺序就是失灵交接顺序，第一条是选用时切换到的当前路由。': 'The order in a group is the failover order. Selecting a group switches to its first route.',
  '设为应用全局接力名单': 'Set as app-wide failover list',
  '路由组 {n}': 'Route group {n}',
  '路由组名称': 'Route group name',
  '这一组还没有路由。': 'This group has no routes yet.',
  '选用方式：输入框左下角的模型选择器，或右侧配置面板的「失灵交接名单」。': 'Select a group from the model picker at the bottom left of the input box, or from “Failover order” in the right configuration panel.',
  // 本机 AI 大脑与任务领取
  'wickrunAI 路由': 'wickrunAI routes',
  '{client} 大脑来源': '{client} brain source',
  '{client} 大脑模型': '{client} brain model',
  '全局改用订阅': 'Use subscription globally',
  '全局路由经本机代理 127.0.0.1:{port} 转发，需要 wickrunAI 保持运行。': 'The global route goes through the local proxy at 127.0.0.1:{port}. Keep wickrunAI running.',
  '大脑': 'Brain',
  '已写入 {file}（首次接管前的原文件另存为 .before-wickrun）。': 'Written to {file}. The original file from before the first takeover is saved as .before-wickrun.',
  '把当前大脑应用到全局': 'Apply this brain globally',
  '沿用 Claude Code 自己的配置': 'Use Claude Code\'s own configuration',
  '终端里的 {client}': '{client} in the terminal',
  '请求经 wickrunAI 本机代理转换后发往「{name}」；密钥留在 wickrunAI，思考强度按映射表下发。': 'Requests are converted by the wickrunAI local proxy and sent to “{name}”. The key stays in wickrunAI, and effort follows the mapping table.',
  '还原接管前的配置': 'Restore the previous configuration',
  '默认只在 wickrunAI 启动时临时注入，不改你的配置文件。需要在终端直接使用同一个大脑时再应用到全局。': 'By default the brain is injected only when wickrunAI starts the client, and your configuration files are untouched. Apply it globally to use the same brain in the terminal.',
  '从对话发送的任务会排队。Claude 连接在线时，在 Claude 里说「领取灯芯AI 任务」即可领取；也可以把下面的指令设为 Claude Desktop 的定时任务，自动领取。': 'Tasks sent from a conversation are queued. When Claude is connected, say “claim a wickrunAI task” in Claude to pick one up, or set the instruction below as a Claude Desktop scheduled task to claim automatically.',
  '复制领取指令': 'Copy claim instruction',
  '已复制领取指令。': 'Claim instruction copied.',
  'Anthropic 原生': 'Anthropic native',
  'OpenAI 兼容（自动转换）': 'OpenAI compatible (auto-converted)',
  '大多数路由是 OpenAI 兼容，由 wickrunAI 本机代理转换；只有端点本身支持 Anthropic /v1/messages 时才选原生，请求会原样转发。': 'Most routes are OpenAI compatible and are converted by the wickrunAI local proxy. Choose native only when the endpoint itself supports Anthropic /v1/messages; requests are then forwarded unchanged.',
  '给 Claude Code / Codex 当大脑时的接口协议': 'Protocol when used as the Claude Code / Codex brain',
  '本机 Claude 订阅（官方登录）': 'Local Claude subscription (official login)',
  '本机 ChatGPT 订阅（官方登录）': 'Local ChatGPT subscription (official login)',
  '订阅': 'Subscription',
  '已接管': 'Managed',
  '没有需要还原的内容。': 'Nothing to restore.',
  '调用 wickrun_claim_task 领取一个灯芯AI 任务并完成。没有任务就回复空闲并停止。完成后用 wickrun_submit_result 交回结果；缺权限、缺信息或能力不够时用 wickrun_report_blocked 说明原因。每次只处理一个任务。': 'Call wickrun_claim_task to claim one wickrunAI task and complete it. If nothing is queued, reply idle and stop. When done, return the result with wickrun_submit_result; if permission, information or capability is missing, explain with wickrun_report_blocked. Handle only one task at a time.',
  '可用 ChatGPT 订阅，或把 wickrunAI 里的任一路由当作 Codex 的大脑。': 'Use a ChatGPT subscription, or any wickrunAI route as the Codex brain.',
  '可用 Claude 订阅、现有配置，或把 wickrunAI 里的任一路由当作 Claude Code 的大脑。': 'Use a Claude subscription, the existing configuration, or any wickrunAI route as the Claude Code brain.',
  '正在替换…': 'Replacing…',
