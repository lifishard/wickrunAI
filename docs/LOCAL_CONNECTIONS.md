...
| Claude Desktop | Login and tool consent remain in the official desktop app. One-click setup adds the local `wickrun_ai` MCP server while preserving other servers. | The model and effort can only be chosen in Claude Desktop itself; wickrunAI's model, effort and route settings do not apply (a dialog explains this on first selection). Workers use explicitly selected API profiles and model IDs. | A dedicated collaboration panel sends tasks by official deep link. Claude calls MCP tools to delegate, inspect worker results, report progress and submit the final answer back to wickrunAI. |
...
**Install as Claude extension (recommended, 2.19.3).** wickrunAI builds `wickrun-ai.mcpb` on the spot (MCPB manifest 0.3: the bundled stdio bridge plus the path of its private connection file) and hands it to Claude Desktop, which shows its own installer and runs the server with its bundled Node.js. This avoids depending on a local Node installation and on which `claude_desktop_config.json` the installed Claude build reads. Installing the extension removes any `wickrun_ai` entry that wickrunAI wrote to the config files, so Claude does not see two copies of the same tools. The extension appears under **Settings → Extensions**; the config-file method appears under **Settings → Developer**. Neither belongs in **Connectors → Add custom connector**, which accepts only public HTTPS MCP servers.
...
## Confirm each change first (2.19.6)

With **Settings → Tools → 逐项修改前确认** on, Grok is the only on-device client that can use work mode. Each Grok edit request carries its full input, so wickrunAI computes the diff and shows it in the code review dialog before Grok writes; at the end of the turn it checks the written files match the approved diffs and flags any unreviewed change. Commands are declined, as with the API file tools in review mode. Claude Code, Codex, Kimi and Claude Desktop write without passing changes through wickrunAI, so they still cannot use work mode; a dialog now explains why and what to do instead before the turn pauses.
